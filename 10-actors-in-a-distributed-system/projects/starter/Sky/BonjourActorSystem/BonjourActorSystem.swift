...
final class BonjourActorSystem: DistributedActorSystem, ObservableObject, @unchecked Sendable {
  ...
  init(localName: String) {
    ...
    self.localActor = ScanActor(name: localName, actorSystem: self)
...
extension BonjourActorSystem {
  ...
  func connectivityChangedFor(deviceName name: String, to connected: Bool) {
    ...
    if connected {
      if let remoteActor = try? ScanActor.resolve(id: name, using: self) {
        withActors { $0[remoteActor.id] = remoteActor }
      }
    } else {
      withActors { $0.removeValue(forKey: name) }
      NotificationCenter.default.post(name: .disconnected, object: name)
    }
  }
  
  func firstAvailableActor() async throws -> ScanActor {
    while true {
      for nextID in withActors(\.keys) {
        guard let nextActor = try? ScanActor.resolve(id: nextID, using: self),
              try await nextActor.count < 4 else {
          continue
        }
